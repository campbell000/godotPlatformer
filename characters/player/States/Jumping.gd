...
func update(player: Player, delta: float):
	...
	if player.is_on_floor():
		...
		groundState.update(player, delta)
	
	var goToWallDrag = false
	if player.velocity.y >= 0 && (player.collidedWithLeftWall() && Input.is_action_pressed("move_left") || player.collidedWithRightWall() && Input.is_action_pressed("move_right")):
